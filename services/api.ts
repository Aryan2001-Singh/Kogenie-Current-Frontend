export const storeAd = async (adData: any, userEmail: string) => {
  ...
  try {
    ...
    const response = await fetch("https://web-production-7b70.up.railway.app/api/ads/store", {
      method: "POST",
      credentials:"include",
...
export const fetchAds = async () => {
  try {
    const response = await fetch("https://web-production-7b70.up.railway.app/api/ads");
...
export const generateAd = async (adData: any) => {
  try {
    const response = await fetch("https://web-production-7b70.up.railway.app/generateAdPrompt", {
      method: "POST",
      credentials:"include",
      headers: { "Content-Type": "application/json" },
